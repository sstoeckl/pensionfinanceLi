...
optimalwc <- function(initial_values,upper_bounds,lower_bounds,ret_age,c2,nu2,nu3,ra,delta,beta,c_age,gender,gender_mortalityTable,w0,
                      CF,li,lg,c1,s1,s2,s3,w2,rho2,rho3,ret,retr,psi,verbose=FALSE,trace=0){
  res <- optimx(par=initial_values,
                fn=.util_optim_wc,gr=NULL,hess=NULL,#gr=function(x) pracma::gradient(util_optim,x),
                #lower=lower_bounds,
                #upper=upper_bounds,
                control=list(all.methods=TRUE, save.failures=TRUE, trace=trace),itnmax=50*9,
  ...
  #control = list(factr=1e-10,maxit=50*9,trace=trace))
  return(res)
}

...
outALL <- tibble(type=numeric(),gend=numeric(),opt=character(),c=numeric(),c2=numeric(),nu2=numeric(),nu3=numeric(),alpha=numeric(),
                 ...
                 method=character(),util=numeric(),fevals=numeric(),convergence=numeric(),nSim=numeric())
for (type in 1:3){
  for (gend in 0:1){
    ...
    # 1) Full Scale Optimization
      ...
      outLC <- optimalLC(initial_values = c(0.5,0.1,0.5,0.5,1,rep(0,3),70),upper_bounds=NULL,lower_bounds=NULL,
                       #ret_age=ret_age,
        ...
        toc()
        beep()
        names(outLC)[3:7] <- c("c","c2","nu2","nu3","alpha"); names(outLC)[c(11,12)] <- c("ret_age","util")
      outALL <- bind_rows(outALL,c("opt"="LC","type"=type,"gend"=gend,outLC[c("c","c2","nu2","nu3","alpha")],makeweights(outLC[c("p6","p7","p8")]),
                                   outLC[c("ret_age","method")],outLC[c("util","fevals","convergence")],"nSim"=nSim))
    ...
    outALL <- bind_rows(outALL,c("opt"="wc","type"=type,"gend"=gend,outwc["c"],"c2"=c2,"nu2"=nu2,"nu3"=nu3,outwc["alpha"],
                                 makeweights(outwc[c("p3","p4","p5")]),"ret_age"=ret_age,
    ...
    outALL <- bind_rows(outALL,c("opt"="w","type"=type,"gend"=gend,"c"=cc,"c2"=c2,"nu2"=nu2,"nu3"=nu3,"alpha"=aalpha,
                                 makeweights(outw[c("p1","p2","p3")]),"ret_age"=ret_age,
...
library(tidyverse)
#data(ret);data(retr)
tw3 <- unlist(outwc[2,5:7]+0.01)
warnings<-TRUE
# research pathological case
util(ret_age=ret_age,c=outwc[2,3],c2=c2,nu2=nu2,nu3=nu3,tw3=tw3,#unlist(outw["Nelder-Mead",1:3]),
          ra=20000,delta=delta,alpha=outwc[2,4],beta=bbeta,c_age=c_age,gender=gender,gender_mortalityTable=gender_mortalityTable,
...
###############
### UTILITY
tcf <- totalCF(ret_age = ret_age, w3 = w3, c = outwc[2,3], c2 = c2,
                   nu2 = nu2, nu3 = nu3, c_age = c_age, gender = gender,
                   w0 = w0, li = li, lg = lg, c1 = c1, s1 = s1, s2 = s2, s3 = s3,
                   w2 = w2, rho2 = rho2, rho3 = rho3, ret = ret, retr = retr, psi = psi, alpha = outwc[2,4], warnings=warnings)
    #########################################
    ## 2. prepare discount factors and survival probabilities
    delta_vec <- (1-delta)^(seq(1,(122-c_age+1))); names(delta_vec) <- (c_age):122
